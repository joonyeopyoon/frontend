...
const AppLayout = ({ children, isSignup = false }: Props): JSX.Element => {
  ...
  const handleImgChange = useCallback(
    async (event) => {
      ...
      try {
        setIsUpdated(false);
        const newUserResponse = await axios.post(`${api.users}/upload`, form, {
        ...
        login({ ...newUserResponse.data.data, token: me.token });
    ...
    <>
      <ZLayout>
        {me && (
          ...
          >
            <ZCard hoverable cover={<AntdImage alt="user" src={me.imgUrl} />}>
        ...
        <Layout>
          ...
          <ZFooter>User ©2021 Created by wwjoon</ZFooter>
        </Layout>
      </ZLayout>
    </>
  );
